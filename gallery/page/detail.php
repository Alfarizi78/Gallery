...
// Check if the delete request is present
if (isset($_GET['delete'])) {
    $delete_id = $_GET['delete'];
    $user_id = $_SESSION['user_id'];

    // Check if the comment belongs to the logged-in user
    $comment_check = mysqli_query($conn, "SELECT * FROM komentar WHERE KomentarID='$delete_id' AND UserID='$user_id'");
    if (mysqli_num_rows($comment_check) > 0) {
        // Delete the comment
        $delete_query = mysqli_query($conn, "DELETE FROM komentar WHERE KomentarID='$delete_id'");
        if ($delete_query) {
    ...
    <div class="row">
        ...
        <div class="col-6">
            ...
            <?php foreach ($komen as $komens): ?>
                ...
                <?php if ($komens['UserID'] == $_SESSION['user_id']): ?>
                <a href="?url=detail&id=<?= $_GET['id'] ?>&delete=<?= $komens['KomentarID'] ?>" class="btn btn-success btn-sm">Delete</a>
                <?php endif; ?> 
               <?php endforeach; ?>
        </div>
    </div>
